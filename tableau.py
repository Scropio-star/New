...
def _is_fol_atom(fmla):
    if len(fmla) != 6:
        return False
    if fmla[0] not in PREDICATES:
        return False
    if fmla[1] != '(':
        return False
    if fmla[2] not in FOL_VARS:
        return False
    if fmla[3] != ',':
        return False
    if fmla[4] not in FOL_VARS:
        return False
    if fmla[5] != ')':
        return False
    return True
                ...
                if f.startswith('~'):
                    ...
                    if inner_kind in (5, 8):
                        ...
                        if c == '\\/' or c == '->':
                            ...
                            progressed = True
                            break
                    if f.startswith('~A') and parse(f[2:]):
                        transformed = 'E' + f[2] + '~' + f[3:]
                        branch['expanded'].add(f)
                        branch['formulas'].append(transformed)
                        progressed = True
                        break
                    if f.startswith('~E') and parse(f[2:]):
